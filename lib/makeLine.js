...
module.exports = () => {
	...
	return scraperjs.StaticScraper.create(`https://veekun.com/dex/pokemon/${props.pkmn}`)
		.scrape($ => (
			'https://veekun.com/'+$('#dex-header > img')[0].attribs.src
		))
		.then(image => (
			{
				image: image,
		...
		.catch(err => {
			console.error(`Failed fetching ${props.pkmn} `)l
			throw err;
		})
};
